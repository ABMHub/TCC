...
def get_training_data(videos_path : str, align_path : str, batch_size = 1, val_size : float = 0.2, data_augmentation=True, validation_only = False):
  ...
  if config is None:
    ...
    train = BatchGenerator((videos[0], aligns[0]), batch_size, augmentation=data_augmentation, mean_and_std=None)
    ...
    train = BatchGenerator(train_data, batch_size, augmentation=data_augmentation, mean_and_std=(config.mean, config.std))
    val = None if val_size is None or val_size == 0 else BatchGenerator(validation_data, batch_size, augmentation=False, preserve_strings=True, mean_and_std=(train.mean, train.std_var))
